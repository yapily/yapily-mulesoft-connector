...
public abstract class GetConsentsOperationBase extends BaseRestOperation {
  protected static final String OPERATION_PATH = "/consents";
  ...
  public GetConsentsOperationBase(ExpressionLanguage arg0) {
    super(arg0);
  }

  public GetConsentsOperationBase() {
    super();
  }

  protected void getConsents(
  ...
  protected void getConsentsMain(
      RestConfiguration config,
      RestConnection connection,
      List<String> filterapplicationUserIdQueryParams,
      List<String> filteruserUuidQueryParams,
    ...
    RestRequestBuilder builder =
        getRequestBuilderWithBindings(
            ...
            .addQueryParam(
                "filter[userUuid]",
                filteruserUuidQueryParams.stream()
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList()))
            .addQueryParam(
                "filter[institution]",
                filterinstitutionQueryParams.stream()
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList()))
            .addQueryParam(
                "filter[status]",
                filterstatusQueryParams.stream()
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList()))
